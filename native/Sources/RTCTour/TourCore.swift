...
public struct ContextFile: Codable, Equatable, Sendable {
    ...
    public init(path: String, oldPath: String? = nil, status: ChangeStatus, additions: Int, deletions: Int,
...
public struct ContextPackBuilder: Sendable {
    ...
    public func build(manifest: ReviewManifest) throws -> ContextPack {
        guard manifest.schemaVersion == RTCConstants.schemaVersion,
              ...
              manifest.files.count <= RTCConstants.maxFiles else {
        ...
        for (index, artifact) in manifest.files.enumerated() where !artifact.hunks.isEmpty {
            ...
            let candidateOmissions = omissions.filter { $0.path != artifact.path || $0.reason != "hunks omitted by input budget" }
            let candidate = try sizedPack(revision: manifest.revision, files: candidateFiles, omissions: candidateOmissions)
    ...
    private func contextFile(_ file: DiffArtifact, includeHunks: Bool) -> ContextFile {
        ContextFile(path: file.path, oldPath: file.oldPath, status: file.status,
    ...
    private func omissionRecords(for file: DiffArtifact) -> [ContextOmission] {
        ...
        if !file.hunks.isEmpty {
            ...
            result.append(ContextOmission(path: file.path, reason: "hunks omitted by input budget",
        ...
        if file.truncated {
            result.append(ContextOmission(path: file.path, reason: "truncated artifact content unavailable", omittedBytes: 0))
        }
        return result
    }

    private func sizedPack(revision: RevisionIdentity, files: [ContextFile], omissions: [ContextOmission]) throws -> ContextPack {
        var size = 0
        for _ in 0..<4 {
...
public struct TourValidator: Sendable {
    public init() {}
    public func validate(_ tour: TourDocument, against revision: RevisionIdentity,
                         expectedInputDigest: SHA256Digest? = nil,
                         anchors: AnchorArtifactSource) async -> Result<TourDocument, TourValidationFailure> {
        ...
        if let expectedInputDigest, tour.inputDigest != expectedInputDigest { issues.append(.init(code: .insufficientGrounding, location: "inputDigest", message: "tour was not generated from the canonical context pack")) }
        ...
        if tour.chapters.count > RTCConstants.maxChapters || tour.reviewFocuses.count > RTCConstants.maxFocuses || tour.risks.count > RTCConstants.maxFocuses || blockCount > RTCConstants.maxBlocks { issues.append(.init(code: .limitExceeded, location: "document", message: "tour limits exceeded")) }
        if (try? RTCCanonicalJSON.encode(tour).count) ?? Int.max > RTCConstants.maxDocumentBytes { issues.append(.init(code: .limitExceeded, location: "document", message: "encoded tour exceeds document limit")) }
        var ids = Set<String>(); var diagramIDs = Set<String>(); var cited = 0; var diagrams = 0; var proseBytes = 0
        ...
        func checkString(_ value: String, _ location: String) {
            let pattern = #"(?i)(<\s*/?\s*(?:[a-z][a-z0-9:-]*|!doctype)\b[^>]*>|\b(?:javascript|vbscript|data|file):|\bhttps?://|```\s*mermaid\b|\bmermaid\s+(?:flowchart|graph|sequenceDiagram|stateDiagram|classDiagram|erDiagram)\b|<\s*svg\b|<\s*script\b|\$\()"#
            if value.range(of: pattern, options: .regularExpression) != nil {
                issues.append(.init(code: .prohibitedContent, location: location, message: "untrusted structured text contains prohibited content"))
            }
        }
        func checkLabel(_ value: String, _ location: String) {
            checkString(value, location)
            if value.count > RTCConstants.maxLabelCharacters { issues.append(.init(code: .limitExceeded, location: location, message: "label exceeds character limit")) }
        }
        func checkDiagram(_ diagram: DiagramDocument, _ location: String) async {
            diagrams += 1
            if !diagramIDs.insert(diagram.id.value).inserted { issues.append(.init(code: .duplicateID, location: location, message: "duplicate diagram ID")) }
            do { _ = try DiagramValidator.validate(diagram) }
            catch { issues.append(.init(code: .invalidDiagram, location: location, message: "diagram IR is invalid or exceeds bounds")) }
            checkLabel(diagram.id.value, location + ".id"); checkLabel(diagram.title.value, location + ".title"); checkText(diagram.summary, location + ".summary")
            ...
            for (index, node) in diagram.nodes.enumerated() {
                ...
                checkLabel(node.id.value, nodeLocation + ".id"); checkLabel(node.label.value, nodeLocation + ".label")
                if node.anchors.isEmpty { issues.append(.init(code: .insufficientGrounding, location: location, message: "diagram node lacks an anchor")) }
            ...
            for (index, edge) in diagram.edges.enumerated() {
                ...
                if edge.anchors.isEmpty { issues.append(.init(code: .insufficientGrounding, location: location, message: "diagram edge lacks an anchor")) }
            ...
            for (index, group) in diagram.groups.enumerated() {
                checkLabel(group.id.value, location + ".group[\(index)].id"); checkLabel(group.label.value, location + ".group[\(index)].label")
            }
        }
        func block(_ block: TourBlock, _ location: String) async {
            ...
            case .diffSlice(let slice):
                guard slice.hunkIndex >= 0, slice.startLine > 0, slice.endLine >= slice.startLine,
                      !slice.path.hasPrefix("/"), !slice.path.split(separator: "/").contains("..") else {
                    issues.append(.init(code: .invalidStructure, location: location, message: "invalid diff slice")); return
                }
                do {
                    let reference = try ReviewAnchor(revision: revision, path: slice.path, scope: .hunk,
        ...
        for (i, focus) in tour.reviewFocuses.enumerated() { checkLabel(focus.title.value, "focus[\(i)].title"); checkText(focus.body, "focus[\(i)]"); if focus.anchors.isEmpty { issues.append(.init(code: .insufficientGrounding, location: "focus[\(i)]", message: "focus lacks a source anchor")) }; for anchor in focus.anchors { await checkAnchor(anchor, "focus[\(i)]") } }
        for (i, chapter) in tour.chapters.enumerated() { checkLabel(chapter.id.value, "chapter[\(i)].id"); checkLabel(chapter.title.value, "chapter[\(i)].title"); if !ids.insert(chapter.id.value).inserted { issues.append(.init(code: .duplicateID, location: "chapter[\(i)]", message: "duplicate chapter ID")) }; checkText(chapter.summary, "chapter[\(i)].summary"); if chapter.anchors.isEmpty { issues.append(.init(code: .insufficientGrounding, location: "chapter[\(i)]", message: "chapter lacks a source anchor")) }; for anchor in chapter.anchors { await checkAnchor(anchor, "chapter[\(i)]") }; for (j, value) in chapter.blocks.enumerated() { await block(value, "chapter[\(i)].block[\(j)]") } }
        for (i, risk) in tour.risks.enumerated() { checkLabel(risk.title.value, "risk[\(i)].title"); checkText(risk.body, "risk[\(i)]"); for anchor in risk.anchors { await checkAnchor(anchor, "risk[\(i)]") } }
        if cited > RTCConstants.maxAnchors || diagrams > RTCConstants.maxDiagrams || proseBytes > RTCConstants.maxProseBytes { issues.append(.init(code: .limitExceeded, location: "document", message: "citation, prose, or diagram limit exceeded")) }
...
public struct FallbackTourProvider: TourProvider {
    ...
    public func generate(request: TourGenerationRequest, progress: @Sendable (TourProgress) async -> Void) async throws -> TourDocument {
        ...
        return try TourDocument(id: deterministicID(revision: request.revision, digest: request.contextDigest), revision: request.revision, producer: .workerSupplied, inputDigest: request.contextDigest, title: "Generated outline unavailable", overview: [.paragraph(overview)], reviewFocuses: [], chapters: chapters, risks: [])
    }
    private func chapterBlocksFor(_ file: DiffArtifact) -> [TourBlock] {
        guard let hunk = file.hunks.first else { return [] }
        let start = hunk.newLines > 0 ? hunk.newStart : hunk.oldStart
        let count = hunk.newLines > 0 ? hunk.newLines : hunk.oldLines
        return [.diffSlice(DiffSliceReference(path: file.path, hunkIndex: 0,
...
public struct TourCoordinator: Sendable {
    ...
    public func run(request: TourGenerationRequest, provider: TourProvider, anchors: AnchorArtifactSource, fallback: TourProvider, progress: @Sendable (TourProgress) async -> Void) async -> Result<TourDocument, RTCError> {
        do { await progress(TourProgress(phase: "generating", fraction: 0)); let candidate = try await provider.generate(request: request, progress: progress); guard !Task.isCancelled else { throw CancellationError() }; switch await validator.validate(candidate, against: request.revision, expectedInputDigest: request.contextDigest, anchors: anchors) { case .success(let tour): await progress(TourProgress(phase: "validated", fraction: 1)); return .success(tour); case .failure: break } }
        catch is CancellationError { return .failure(RTCError(code: .internalError, message: "cancelled", retryable: true)) }
        catch { }
        do { let candidate = try await fallback.generate(request: request, progress: progress); switch await validator.validate(candidate, against: request.revision, expectedInputDigest: request.contextDigest, anchors: anchors) { case .success(let tour): return .success(tour); case .failure: return .failure(RTCError(code: .insufficientGrounding, message: "tour and fallback could not be grounded", retryable: false)) } }
