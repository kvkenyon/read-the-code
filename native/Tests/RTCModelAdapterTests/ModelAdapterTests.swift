...
final class ModelAdapterTests: XCTestCase {
    ...
    func testOllamaFragmentedStreamIsReassembledAndBounded() async throws {
        let transport = StubTransport(chunks: [
            Data("{\"response\":\"{\\\"ok\\\":\"".utf8), Data("yes\\\"}\",\"done\":false}\n".utf8),
            Data("{\"response\":\"\",\"done\":true}\n".utf8),
        ])
        let adapter = OllamaAdapter(
            endpoint: try LoopbackEndpoint(URL(string: "http://127.0.0.1:11434")!), model: "test", transport: transport)
        let result = try await adapter.generateStructured(request: Data("{}".utf8), schema: Data("{}".utf8))
    ...
    func testCredentialIsOnlyAddedToOpenAIRequest() async throws {
        ...
        let adapter = OpenAICompatibleAdapter(
            endpoint: try LoopbackEndpoint(URL(string: "http://127.0.0.1:8080")!), model: "local",
